...
def main(tasks, task, dataset, device, train_epoch, seed, fold, batch_size, rate, split, modelpath, logger, lr, attn_head, output_dim, attn_layers, dropout, mean, stds, D, useedge, met, savem, save_splits=False):
    ...
    if task == 'clas':
        ...
        for fol in range(1, fold+1):
            ...
            train_loader, valid_loader, test_loader = load_data(dataset, batch_size, rate[0], rate[1], 0, task, split, seed_, save_splits=save_splits)
            ...
            if valid_auc > best_val_auc:
                ...
                if savem:
                    ...
                    # Save model state and normalization parameters together
                    torch.save({
...
if __name__ == '__main__':
    ...
    parser.add_argument('mode', type=str, choices=['train', 'test', 'search', 'predict'], help='train, test or hyperparameter_search')
    ...
    parser.add_argument('--save_splits', action='store_true', help='保存数据集划分为CSV文件')
    ...
    if args.mode == 'search':
        ...
        fold_result = main(numtasks, task, moldata, device, args.train_epoch, args.seed, args.fold, args.batch_size, rate, args.split, modelpath, logger, args.lr, attn_head, args.output_dim, args.attn_layers, args.dropout, mean, std, args.D, useedge, args.metric, True, args.save_splits)
    elif args.mode == 'test':
        ...
        assert (args.pretrain is not None)
        mean, std = None, None
        fold_result = test(tasks=numtasks, task=task, dataset=moldata, device=device, seed=args.seed, batch_size=args.batch_size, logger=logger, attn_head=attn_head, output_dim=args.output_dim, attn_layers=args.attn_layers, dropout=args.dropout, pretrain=args.pretrain, mean=mean, stds=std, D=args.D, useedge=useedge, met=args.metric)
    else:pass
